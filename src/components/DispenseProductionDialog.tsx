  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg p-0 overflow-hidden bg-background flex flex-col max-h-[92dvh] sm:max-h-[90vh]">
        ...
        <div className="bg-gradient-to-r from-cocoa to-brand p-4 sm:p-5 ltr:pe-10 rtl:ps-10 text-white shrink-0">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              ...
              <div className="flex size-8 sm:size-9 items-center justify-center rounded-lg bg-white/15 text-white backdrop-blur-sm border border-white/20">
                <Factory className="size-4 sm:size-5" />
              </div>
              <div>
                <DialogTitle className="text-base sm:text-xl font-bold text-white leading-tight">
                  {t("dispenseToProduction")}
                </DialogTitle>
          ...
          <div className="mt-3 rounded-lg bg-black/20 p-2.5 sm:p-3 backdrop-blur-sm border border-white/15 grid grid-cols-2 sm:flex sm:flex-wrap items-center justify-between gap-2 text-xs">
            <div>
              <span className="text-white/70 block text-[10px] sm:text-xs">{lang === "ar" ? "المادة الخام" : "Material"}</span>
              <span className="font-bold text-xs sm:text-sm text-white truncate block">{item.name}</span>
            </div>
            {item.batch_number && (
              <div>
                <span className="text-white/70 block text-[10px] sm:text-xs">{lang === "ar" ? "رقم التشغيلة" : "Batch #"}</span>
                <span className="font-mono font-semibold text-amber-200 text-xs sm:text-sm">#{item.batch_number}</span>
              </div>
            )}
            <div>
              <span className="text-white/70 block text-[10px] sm:text-xs">{t("currentStockBalance")}</span>
              <span className="font-bold text-white font-mono text-xs sm:text-sm">
                {currentQty} {item.unit || "كجم"}
              </span>
            </div>
            <div>
              <span className="text-white/70 block text-[10px] sm:text-xs">{t("expiryDate")}</span>
        ...
        <form id="dispense-form" onSubmit={handleDispenseSubmit} className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 overscroll-contain">
        ...
        <DialogFooter className="p-3 sm:p-4 border-t border-border bg-card/95 backdrop-blur-sm flex flex-row items-center justify-end gap-2 shrink-0 pb-[max(0.75rem,env(safe-area-inset-bottom,16px))]">
          <Button
            type="button"
            variant="outline"
            onClick={() => onOpenChange(false)}
          ...
          <Button
            ...
            className="bg-brand hover:bg-brand/90 text-white font-semibold text-xs gap-1.5 min-w-[130px] h-9 px-4 shadow-sm"
          >
            {loading ? (
              <span>{t("saving")}</span>
            ) : (
