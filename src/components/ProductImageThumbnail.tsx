...
import { ImageIcon, ZoomIn, Sparkles, Images } from "lucide-react";
...
export function ProductImageThumbnail({
  ...
  const buttonElement = (
    ...
    >
      ...
      {photoCount > 1 && (
        <span className="absolute bottom-0 end-0 flex items-center gap-0.5 rounded-tl bg-black/80 px-1 py-0.5 text-[9px] font-bold text-amber-300 shadow-xs">
  ...
  // On touch devices / mobile, render direct button without HoverCard interference
  if (!canHover) {
  ...
  return (
    <HoverCard openDelay={200} closeDelay={150}>
      ...
      <HoverCardContent
        ...
        align="center"
        sideOffset={10}
        className="w-72 overflow-hidden rounded-xl border border-brand/25 bg-card/95 p-0 shadow-2xl backdrop-blur-xl animate-in fade-in-0 zoom-in-95 duration-200"
      >
        ...
        <div className="border-t bg-muted/30 p-3">
          ...
          <div className="mt-2 flex items-center justify-between border-t pt-1.5 text-[10px] text-muted-foreground">
            <span className="flex items-center gap-1 text-brand font-medium">
              ...
              {photoCount > 1 ? `${photoCount} ${t("photoCountLabel")}` : t("hoverToZoom")}
            </span>
